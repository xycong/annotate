...
#---
defmodule Annotate.AuthTest do
  use Annotate.ConnCase
  alias Annotate.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Annotate.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Annotate.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Annotate.User{id: 123})
      |> send_resp(:ok, "")
